...
function DetailMotorbike(props) {
  ...
  const getMotorBike = useCallback(() => {
    getVehicleApi(id)
      .then((res) => {
        // console.log(res);
        if (res.status === 200) {
          setvehicleMotorbike(res.data.result[0]);
        ...
        {vehicleMotorbike.length === 0 ? (
          ...
          <div className="col-lg-6 wrapper-text-detail col-md-10 mt-lg-0 mt-md-5">
            <h1 className="title-detail">
              {vehicleMotorbike.name} <br />
              <span className="detail-title-location">
                {vehicleMotorbike.location}
              </span>
            </h1>
            ...
            <div className="detail-vehicle">
              <p>Capacity : {vehicleMotorbike.capacity} person</p>
              <p>Type : {vehicleMotorbike.category}</p>
            ...
            <div className="price">
              <p>
                Rp.
                {vehicleMotorbike.length !== 0
                  ? numberToRupiah(vehicleMotorbike.price)
                  : " "}
                /day
              </p>
            </div>
...
export default DetailMotorbike;
